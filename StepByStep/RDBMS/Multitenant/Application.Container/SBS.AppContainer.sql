-- Primero creo una PDB como Application Container !!!

create pluggable database APPCONT as application container from PDB1 keystore identified by "AaZZ0r_cle#1";

SQL> create pluggable database APPCONT as application container from PDB1 keystore identified by "AaZZ0r_cle#1";
...
--- Colgando del application container, creo dos application PDB !!!
...
SQL> CREATE PLUGGABLE DATABASE apppdb1 ADMIN USER pdb_admin IDENTIFIED BY "AaZZ0r_cle#1";

Pluggable database created.

SQL> CREATE PLUGGABLE DATABASE apppdb2 ADMIN USER pdb_admin IDENTIFIED BY "AaZZ0r_cle#1";
...
sqlplus sys/"AaZZ0r_cle#1"@dbcn01:1521/apppdb2.sub02261644010.skynet.oraclevcn.com as sysdba

ADMINISTER KEY MANAGEMENT SET KEY FORCE KEYSTORE IDENTIFIED BY "AaZZ0r_cle#1" WITH BACKUP;


--- En el App Container, creo un esquema:

sqlplus sys/"AaZZ0r_cle#1"@dbcn01:1521/appcont.sub02261644010.skynet.oraclevcn.com as sysdba
create user APPUSER identified by "AaZZ0r_cle#1" default tablespace USERS temporary tablespace TEMP;
...
--- Este esquema es un COMMON USER, porque lo he creado en un Application Container !!!
...
--- En este esquema, creo tablas, vistas, procedures !!!

sqlplus APPUSER/"AaZZ0r_cle#1"@dbcn01:1521/appcont.sub02261644010.skynet.oraclevcn.com
...
---- Ahora tengo que publicar la aplicación compuesta por estos elementos a las APPPDB !!!
--- Esto se hace conectado al APPCONT !!!
...
--- Esta tabla solo se puede modificar en el APPCONT !!!
exec dbms_pdb.set_data_linked (schema_name => 'APPUSER',object_name=>'DIM_COMUN',namespace=>1)

--- extended Data linked => Cada APPPDB puede añadir sus datos !!!
...
---- Ahora hay que sincronizar las APPPDB con el APPCONT !!!

sqlplus / as sysdba
alter session set container=APPPDB1;

--- Primero creo el esquema APPUSER !!!
...
---- Puedo consultar DIM_COMUN, pero no hacer DML:
sqlplus appuser/"AaZZ0r_cle#1"@dbcn01:1521/apppdb1.sub02261644010.skynet.oraclevcn.com
...
---- Puedo consultar DIM_SHARED, y también añadir registros !!!
...
--- Como no se utiliza C3 en el procedure, intento crear mi versión particular del procedure en APPPDB1:
...
--- No puedo compilar, porque el procedure es del APPCONT !!!

--- Intento sobrecargarlo !!!
...
--- => Tampoco !!!
-- No se fija en la firma, es decir que mismo nombre mismo procedure !!!
--- La solución para permitir que cada PDB tenga su versión es que no sea parte de la APP MYAPP, así no se sincroniza !!!
...
--- Esto falla porque los sinonimos y las vistas estan en el mismo namespace !!!

--- Ahora configuro APPPDB2 !!!
...
sqlplus appuser/"AaZZ0r_cle#1"@dbcn01:1521/apppdb2.sub02261644010.skynet.oraclevcn.com
create table FACT_TABLE
(
   C1 number not null,
   C2 VARCHAR2(20),
...
---- Ahora generamos una nueva versión de la aplicación:
--- Desde APPCONT !!!

-- Compilo el procedure con un parametro más, y la vista también !!!
sqlplus / as sysdba
alter session set container=APPCONT;
...
--- Ahora tengo APPPDB1 en 1.1 y APPPDB2 en 1.0 !!!

[oracle@dbcn01 ~]$ sqlplus appuser/"AaZZ0r_cle#1"@dbcn01:1521/apppdb1.sub02261644010.skynet.oraclevcn.com
...
[oracle@dbcn01 ~]$ sqlplus appuser/"AaZZ0r_cle#1"@dbcn01:1521/apppdb2.sub02261644010.skynet.oraclevcn.com
...
--- Ahora puedo ejecutar el procedure desde APPPDB1 !!!!
...
--- En cualquier momento puedo actualizar APPPDB2 a la versión 1.1 !!!
...
sqlplus / a sysdba
alter session set container=APPCONT;
SQL> CREATE PLUGGABLE DATABASE AS SEED FROM APPCONT  keystore identified by "AaZZ0r_cle#1";
...
[oracle@dbcn01 ~]$ sqlplus appuser/"AaZZ0r_cle#1"@dbcn01:1521/appcont$seed.sub02261644010.skynet.oraclevcn.com
