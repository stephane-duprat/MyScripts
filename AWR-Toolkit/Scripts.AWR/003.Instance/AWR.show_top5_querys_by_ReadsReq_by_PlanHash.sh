#!/bin/ksh
# #####################################################################
# $Header: AWR.show_top5_querys_by_ReadsReq_by_PlanHash.sh 30/03/2011 sduprat_es Exp $
#
# AWR.show_top5_querys_by_ReadsReq_by_PlanHash.sh
#
...
#
#    USAGE
#     AWR.show_top5_querys_by_ReadsReq_by_PlanHash.sh <AWR SNAPSHOT> <INSTANCE_NUMBER>
#     AWR.show_top5_querys_by_ReadsReq_by_PlanHash.sh -show => lists the available snapshots in the database !!!
#
#    DESCRIPTION
...
set linesize 500
col "Begin Snap=$bsnap - Instance=$inst_num" format a55

select V.*,
       V.TotalMetricReq/V.TotalExec as ReadsReqPerExec 
from (
SELECT to_char(BEG.END_INTERVAL_TIME,'YYYYMMDDHH24MI') || '-' || to_char(BEG.BEGIN_INTERVAL_TIME,'YYYYMMDDHH24MI') || ',' ||
BEGSTAT.plan_hash_value || ',' as "Begin Snap=$bsnap - Instance=$inst_num",
sum(BEGSTAT.PHYSICAL_READ_REQUESTS_DELTA) as TotalMetricReq,
sum(BEGSTAT.EXECUTIONS_DELTA) as TotalExec,
sum(1) as Totalquerys
