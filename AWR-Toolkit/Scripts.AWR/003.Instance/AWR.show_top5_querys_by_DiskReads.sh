#!/bin/ksh
# #####################################################################
# $Header: AWR.show_top5_querys_by_DiskReads.sh 30/03/2011 sduprat_es Exp $
#
# AWR.show_top5_querys_by_DiskReads.sh
#
...
#
#    USAGE
#     AWR.show_top5_querys_by_DiskReads.sh <AWR SNAPSHOT> <INSTANCE_NUMBER>
#     AWR.show_top5_querys_by_DiskReads.sh -show => lists the available snapshots in the database !!!
#
#    DESCRIPTION
...
#    1.1.0    sduprat_es      13/10/11   - Add dbid as third parameter on the command line
#
# #####################################################################
...
select V.*,
       V.TotalMetricReq/V.TotalExec as DiskReadsPerExec 
from (
...
sum(BEGSTAT.DISK_READS_DELTA) as TotalMetricReq,
