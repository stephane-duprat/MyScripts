#!/bin/ksh
# #####################################################################
# $Header: AWR.show_top5_querys_by_Elapsed.sh 30/03/2011 sduprat_es Exp $
#
# AWR.show_top5_querys_by_Elapsed.sh
#
...
#
#    USAGE
#     AWR.show_top5_querys_by_Elapsed.sh <AWR SNAPSHOT> <INSTANCE_NUMBER>
#     AWR.show_top5_querys_by_Elapsed.sh -show => lists the available snapshots in the database !!!
#
#    DESCRIPTION
...
select V."Begin Snap=$bsnap - Instance=$inst_num" ,
       V.TotalMetric/1000000 as TotalMetric,
       V.TotalExec as TotalExec,
       V.TotalMetric/V.TotalExec/1000000 as ElapsedPerExec
from (
...
sum(BEGSTAT.ELAPSED_TIME_DELTA) as TotalMetric,
