...
#    1.4.0   sduprat_es       01/08/11   - Compute CPU IOWait percentage out of IOWAIT_TIME statistic
#    1.4.1   sduprat_es       04/06/13   - Include process number in the work file name, to allow parallel executions
#    1.4.2   sduprat_es       01/10/13   - Added cell stats - Remove "-" from snap_id, to allow compatibility with conso.sqlite3.pl
#
# #####################################################################
...
col inum format 99
select v.*,'$1 ' || v.bsnap || ' ' || v.esnap || ' ' || v.INUM || ' ' || v.DBID as cmd from (
select B.DBID, B.DB_NAME, B.INSTANCE_NAME as instance, A.INSTANCE_NUMBER as inum, trunc(A.BEGIN_INTERVAL_TIME), min(A.SNAP_ID) as bsnap, max(A.snap_id) as esnap
...
col the_line format a100
col xxx format a5
set appinfo GATHER_DRIVER
alter session set optimizer_mode = 'FIRST_ROWS';
spool $work_file

SELECT to_char(END.END_INTERVAL_TIME,'YYYYMMDDHH24MI') ||
...
do
    ...
    cpu_iow=$(grep "^"$snap $app_file | grep IOWAIT_TIME | awk '{ print $NF }') ## CPU IO Wait in cs
    ...
    cpu_iow_pct=$(echo "scale=0; "$cpu_iow"/("$inter"*"$num_cpu")" | bc -l)
    ##
    ## Write the results in the CPU file !!!
    ##
    echo $snap","${cpu_usr_pct}","${cpu_sys_pct}","${cpu_idl_pct}","${cpu_iow_pct} >> $cpu_file
done
